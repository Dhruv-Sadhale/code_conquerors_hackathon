...
from django.contrib.auth.decorators import login_required
import ast #check
iresponses=[] #these are the options chosen by the user
stored_response="" #these are the questions and options generated by the AI
...
def admin_dashboard(request):
    ...
    clubs = Club_Secondary.objects.all()
    username = request.user.username
    print(username)
    # if not CoreClubAdmin.objects.filter(username=username).exists():
    #     return redirect('home')
    if(username!='dhruvtest' and username!='dhruv8'):
        return redirect('home')
    # Pass clubs to the template
    return render(request, 'base/admin_dashboard.html', {'clubs': clubs})

@login_required(login_url='/accounts/google/login/')
def core_club_admin_view(request, pk):
...
def dashboard(request):
    # Use request.user directly (Google user info available)
    if request.user.is_authenticated:
        # Fetch the user's responses to display on the dashboard
        user_responses = ClubResponse.objects.filter(user=request.user)

        # Fetch the clubs where the user is an inductee
        ...
        return render(request, 'base/dashboard.html', {
            ...
            'username': request.user.username,  # Google account username
            'inducted_clubs': inducted_clubs
        })
    else:
...
def send_email(request):
    if request.method == 'POST':
        subject = "Test"
        message = "Hey! This is an email to test the working of Django mail."
        from_email = 'noreply@demo.com'  # You can also use DEFAULT_FROM_EMAIL
        recipient_list = [
...
def save_response_to_file(response_text, file_name="gemini_response.json"):
    try:
        with open(file_name, "w") as file:
            json.dump(response_text, file, indent=4)
        print(f"Response successfully saved to {file_name}")
    except Exception as e:
...
def gemini_response(request):
    # if request.method == 'POST':
        ...
        prompt_file_path = "base/prompt.txt"
        prompt = read_prompt_from_file(prompt_file_path)
        if not prompt:
            return JsonResponse({"error": "Prompt file not found or empty."})
        if prompt:
            gemini_response = generate_text(prompt)
            gemini_response=gemini_response.replace('\"', '"')
            gemini_response=gemini_response.replace("```", "")
            gemini_response=gemini_response.replace("json", "")
            stored_response = gemini_response
            print(stored_response)
            if gemini_response:
                
                save_response_to_file(gemini_response, "gemini_response.json")

            else:
                return JsonResponse({"error": "No valid JSON found in the response."})
        else:
            return JsonResponse({'error': 'Prompt not provided!'})
        return render(request, 'base/index.html',{'gemini_response': gemini_response})
    # return render(request, 'base/gemini.html')


@csrf_exempt
@login_required(login_url='/accounts/google/login/')
def record_club(request):
    print("in record club---->")
    if request.method == 'POST':
        try:
            data_list = json.loads(request.body)
            print('Received data in record club:', data_list)
            prompt1="you generated the following questions previously:"
            prompt2=str(iresponses)
            prompt3="to which the user gave the following answers:"
            prompt_file_path = "base/answer_prompt.txt"
            prompt = read_prompt_from_file(prompt_file_path)
            ...
            if not answer_prompt:
                return JsonResponse({"error": "Prompt file not found or empty."})
            if answer_prompt:
                gemini_response = generate_text(answer_prompt)
                gemini_response=gemini_response.replace('\"', '"')
                gemini_response=gemini_response.replace("```", "")
                gemini_response=gemini_response.replace("json", "")
                ...
                if gemini_response:
                    ...
                    save_response_to_file(gemini_response, "answer_gemini_response.json")
            ...
            response_dict = ast.literal_eval(gemini_response)
            # response_dict = json.dumps(response_dict)
            clubs_list = list(response_dict.values())
            print(response_dict)
            ...
            if len(user_responses) > 2:
                user_responses_to_delete = list(user_responses[3:])
                ClubResponse.objects.filter(id__in=[obj.id for obj in user_responses_to_delete]).delete()
                
            subject = "Results of Club Navigation Test"

            message = f"""Dear {request.user.username},

            Thank you for participating in the selection test. We have provided you recommendations for the following club(s):

            {chr(10).join(f"• {club}" for club in clubs_list)}

            Our team will review your application if you choose to proceed, and you will be notified about the next steps shortly.
